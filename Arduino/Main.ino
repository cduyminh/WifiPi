...
NewPing sonar(TRIG_PIN, ECHO_PIN, MAX_DISTANCE); //global safety sensor input
Servo myservo;
int distance = 100; //global variable
int pos = 0; //global servo signals
bool forward = 0; //global forward varialble
bool backward = 0;
bool left = 0;
bool right = 0;
...
void setup() {
  ...
  pinMode(8, INPUT); //Go right pin
  pinMode(10, INPUT); //Go backward pin
  pinMode(11, INPUT); //Go left pin
  pinMode(12, INPUT); //Go foward pin
...
void loop() {
  //Always starts off by going forward for 2 seconds
  forward = digitalRead(12);
  backward = digitalRead(10);   // read the forward input pinwdawdaddddddd
  left = digitalRead(11);   // read the forward input pin
  right = digitalRead(8);   // read the forward input pin
...
int lookLeft()
{
  ...
  for (pos = 180; pos >= 90; pos -= 1) { // goes froxm 180 degrees to 90 degrees
    // in steps of 1 degree
    myservo.write(pos);              // tell servo to go to position in variable 'pos'
    delay(5);
  }
